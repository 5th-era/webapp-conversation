...
function containsAllSubstrings(answer: string): boolean {
  ...
  return substrings.every((substring) => answer.includes(substring));
}
const optimizePrompt = "请用我的演讲内容和你给出的点评建议，结合该场景的演讲技巧，帮我写一篇更好的演讲稿。"
        ...
        <div className={`${s.answerWrap}`}>
          <div className={`${s.answer} relative text-sm text-gray-900`}>
            <div className={'ml-2 py-3 px-4 bg-gray-100 rounded-tr-2xl rounded-b-2xl'}>
                ...
                <div className='flex flex-wrap'>
                  {item.suggestedQuestions.filter(q => !!q && q.trim()).map((question, index) => (
                    <div
                      ...
                      onClick={() => onQueryChange(question.replace("【点击查看】", ""))}
                    >
                      {question}
                    </div>),
                  )}
              ...
              {
                containsAllSubstrings(content) && (
                  <div
                    className='mt-1 mr-1 max-w-full last:mr-0 shrink-0 py-[5px] leading-[18px] items-center px-4 rounded-lg border border-gray-200 shadow-xs bg-white text-base font-medium text-primary-600 cursor-pointer'
                    onClick={() => onQueryChange(optimizePrompt)}
                  >
                    【点击查看更好的演讲稿内容】
                  </div>
                )}

...
import { Loading02, XClose } from '@/app/components/base/icons/line/general'
import { audio2text } from '@/service/base'
...
const VoiceInput = ({
  ...
  const handleStopRecorder = useCallback(async () => {
    ...
    try {
      const audioResponse = await audio2text({
        xhr: new XMLHttpRequest(),
        data: formData,
      })
      onConverted(audioResponse.id.startsWith('Request failed with status code') ? "" : audioResponse.id)
      onCancel()
    }
    catch (e) {
      onConverted('')
